...
// ============================
pub static REG_STRIP_STREET_ABBVR0: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE_ABBVR.len() / 4;
    let modified = STREET_TYPE_ABBVR[..size].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET_ABBVR1: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE_ABBVR.len() / 4;
    let modified = STREET_TYPE_ABBVR[size..size*2].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET_ABBVR2: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE_ABBVR.len() / 4;
    let modified = STREET_TYPE_ABBVR[size*2..size*3].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET_ABBVR3: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE_ABBVR.len() / 4;
    let modified = STREET_TYPE_ABBVR[size*3..].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET0: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE.len() / 4;
    let modified = STREET_TYPE[..size].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET1: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE.len() / 4;
    let modified = STREET_TYPE[size..size*2].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET2: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE.len() / 4;
    let modified = STREET_TYPE[size*2..size*3].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
...
pub static REG_STRIP_STREET3: Lazy<Regex> = Lazy::new(||{
    let size = STREET_TYPE.len() / 4;
    let modified = STREET_TYPE[size*3..].iter().map(|street| {
        format!(r"(?:^|\W){}(?:$|\W)$", street.to_ascii_uppercase())
    }).collect::<Vec<String>>();
